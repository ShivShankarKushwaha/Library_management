...
async function admin()
{
    ...
    let responce1= await collection.find({name:process.env.ADMIN}).toArray();
...
app.post("/removebook",async(req,res)=>{
    ...
    if (responce1!="") {
        ...
        console.log(result);
        res.render(__dirname+"/removebooksuccess.ejs",{removebookname});
    }
    else
    {
        res.render(__dirname+"/removebookfailed.ejs",{removebookid});
    }
